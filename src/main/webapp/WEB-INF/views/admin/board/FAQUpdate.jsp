    			...
    			$.ajax({
    				...
    				success : function(res){
    					if(res == "1"){
    						...
    						alert("자주하는 질문 수정에 실패하였습니다.");
    						location.reload();
    					}
    				},
    				error : function(){
    					alert("전송 오류(FAQUpdate.jsp)")
    				}
    			});
    		}
    	}
			...
			<div id="admin-FAQ-right-inner-content">
				<div id="admin-FAQ-menu">
					...
					<div id="FAQ-answer-div">
						<textarea rows="15" name="answer" id="answer" required class="form-control" >${vo.answer }</textarea>
					</div>
					<div id="btn-div">
						<input type="button" value="자주하는 질문 수정"  onclick="FAQUpdateOk()" />
					</div>
				</div>
			</div>
		</div>
