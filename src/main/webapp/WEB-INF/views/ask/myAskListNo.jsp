		...
		<div id="myAskList-right-content">
			...
			<div id="myAskList-right-inner-content">
				<div id="myAskList-myinfo-menu">
					<div id="myAskList-myinfo-menu-str">내 문의 내역 | 답변대기 <i class="fa-solid fa-clipboard-list"></i></div>
					<hr/>
					...
					<div>
						<table class="table table-hover">
							...
							<c:if test="${empty vos}">
								<tr >
									<td colspan="4">답변대기 중인 문의 내역이 존재하지 않습니다.</td>
								</tr>
							</c:if>
							<c:if test="${!empty vos}">
								...
								<c:forEach var="askVO" items="${vos}" varStatus="st">
									<tr>
										<td>${curScrStartNo}</td>
										<td style="text-align: left;"><a href="${ctp}/member/ask/askContent?idx=${askVO.idx}&pag=${pageVO.pag}&pageSize=${pageVO.pageSize}&part=${part}">${askVO.title} <span style="color: #eee"><c:if test="${askVO.imgs != ''}"><i class="fa-solid fa-paperclip"></i></c:if></span></a></td>
										<td>
